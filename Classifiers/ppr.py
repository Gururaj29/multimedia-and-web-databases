...
class PPR:
    ...
    '''
    def __init__(self, random_jump_probability = 0.15, num_walks=100000) -> None:
        ...
        for image_id, features in self.testing_images_features.items():
            image_node = "image#" + image_id
            image_features = np.array(features)
            image_features_2d = image_features.reshape(1, -1)

            self.graph.add_node(image_node)
            
            self.seeds = self.get_seeds(self.graph, image_node, image_features_2d, self.label_features)
            if self.random_jump_probability < 0.5:
                self.seeds = self.seeds[:3]
            else:
                self.seeds = self.seeds[:1]

            pagerank = self.personalized_page_rank(graph=self.graph, seeds=self.seeds, source=image_node, num_walks=num_walks, beta=self.random_jump_probability)
            
            labels_probabilities = {node.split('#')[1]: score for node, score in pagerank.items() if node.startswith('label#')}
            predicted_labels = heapq.nlargest(5, labels_probabilities, key=labels_probabilities.get)
    ...
    def personalized_page_rank(self, graph, seeds, source, num_walks, beta):
        ...
        for i in range(num_walks):
            ...
            if i == 0:
                # remove label node from graph after first walk
